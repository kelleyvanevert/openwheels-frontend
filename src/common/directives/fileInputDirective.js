...
.directive('fileInputChange', function () {
  ...
  return {
    restrict: 'EA',
    template: '<span><input id="{{id}}" type="file" class="inputfile" /><label for="{{id}}" ng-show="!fileProps.sizeOk && !uploaded.sizeOk"><ng-md-icon icon="file_upload" class="text-white"></ng-md-icon> Selecteer een foto</label><span ng-show="uploaded.hasUploaded && uploaded.isImage && fileProps.sizeOk"><inline-spinner /></span></span>',
    replace: true,
    scope: {
      onChange: '=',
      fileProps: '=',
